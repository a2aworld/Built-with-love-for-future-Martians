...
export const AgentInterface: React.FC<AgentInterfaceProps> = ({ messages, isLoading }) => {
  ...
  const [viewImage, setViewImage] = useState<string | null>(null); // Lightbox State
  ...
  return (
    <>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              ...
              {msg.imageUrl && (
                  <div 
                    className="mt-4 max-w-[95%] w-full rounded shadow-md overflow-hidden border border-slate-700/50 bg-black relative animate-fade-in group cursor-zoom-in"
                    onClick={() => setViewImage(msg.imageUrl || null)}
                  >
                      {/* Caption */}
                      <div className="absolute top-3 left-3 bg-black/60 backdrop-blur px-2 py-1 text-[9px] text-white/80 font-sans tracking-widest uppercase rounded-sm border border-white/10 z-10">
                          Fig 1.1: Iconographic Reconstruction
                      </div>
                      
                      {/* Watermark */}
                      <div className="absolute bottom-3 right-3 z-10 opacity-60">
                          <span className="font-serif text-[10px] text-gold-500 tracking-wider font-bold drop-shadow-md bg-black/40 px-2 py-1 rounded">
      ...
      {viewImage && (
        <div 
          className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 animate-fade-in cursor-zoom-out"
        ...
        >
           <div className="relative max-w-full max-h-full">
              <img 
                src={viewImage} 
                alt="Detailed View" 
                className="max-h-[90vh] max-w-[95vw] shadow-2xl border border-slate-700 rounded-sm object-contain"
              />
              
              {/* Full Screen Watermark */}
              <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-md px-4 py-2 rounded border border-gold-500/30">
                  <p className="font-serif text-sm md:text-lg text-gold-500 tracking-widest uppercase font-bold text-center">
                      A2A World | Galactic Storybook
                  </p>
                  <p className="text-[8px] text-slate-400 text-center uppercase tracking-wider mt-1">
